...
MODEL_PRESETS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "model_id": "",
        "api_base": None,
        "api_base_placeholder": "https://api.example.com/v1 或 http://host.docker.internal:11434/v1",
        "model_id_placeholder": "填服务端部署的模型名，如 qwen3:8b / minicpm5-2b",
        "model_family": None,
        "channel_name": "custom_gateway",
        "description": "适合 OpenRouter、智谱等兼容网关，以及本机 Ollama / LM Studio / MLX 等推理服务。",
        "recommended_for": ["本地推理服务", "自定义网关", "备用模型"],
        "requires": ["api_key", "api_base", "model_id"],
        "help": "Provider 保持 OpenAI；填写服务的 API Base、模型名即可。本地服务不校验 Key 时可填 ollama 等占位值。",
        "defaults": {
            **DEFAULT_MODEL_PARAMETERS,
            "name": "OpenAI 兼容网关",
    ...
    },
    {
        ...
        "model_id": "",
        "api_base": None,
        "model_id_placeholder": "如 openai/gpt-4.1-mini（裸模型名按 OpenAI 兼容网关处理）",
        "model_family": None,
        "channel_name": None,
        "description": "适合熟悉 LiteLLM 路由和供应商参数的高级用户。",
        "recommended_for": ["高级配置", "特殊供应商"],
        "requires": ["provider", "model_id"],
        "help": "模型名可填完整 LiteLLM 路由（如 openai/gpt-4.1-mini）；只填裸模型名时会按 OpenAI 兼容网关调用。不确定时优先用上面的预设。",
        "defaults": {
            **DEFAULT_MODEL_PARAMETERS,
            "name": "自定义模型",
