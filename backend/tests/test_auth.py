...
class _ReadOnlyTokenLookupSession:
    ...
    async def execute(self, _statement):
        ...
        expires_at = datetime.now(UTC) + timedelta(days=settings.SESSION_REFRESH_THRESHOLD_DAYS + 1)
        return _FakeSessionLookupResult((101, expires_at, self.user.id))
